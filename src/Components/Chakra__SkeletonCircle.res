...
external make: (
  ...
  /**
    Component Props {SkeletonCircle}
   **/
  ~_as: string=?,
  ~isTruncated: bool=?,
  ~endColor: string=?,
...
let makeProps = (
  ...
  makeProps(
    /**
      Component Props {SkeletonCircle}
     **/
    ~endColor=?endColor->Belt.Option.map(Chakra__AtomicTypes.Colors.toString),
    ~startColor=?startColor->Belt.Option.map(Chakra__AtomicTypes.Colors.toString),
    /**
      Margin and Padding
     **/
